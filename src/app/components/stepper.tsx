...
export default function HorizontalLinearAlternativeLabelStepper() {
  ...
  const [alertaParams, setAlertaParams] = React.useState<AlertaParams>({
    mensagem: "",
    severity: "success",
  });
  ...
  async function handleSalvar() {
    ...
    const payload: Record<string, any> = {
      ...
      tb_adocao_destino_animais_anteriores: toTF(
        Boolean(familia.teveAnimaisAntes)
      ),
      tb_adocao_residencia: moradia.residencia || null,
      tb_adocao_possui_espaco_externo: toTF(
        Boolean(moradia.possuiEspacoExterno)
      ),
      ...
      tb_adocao_disposto_veterinario: toTF(
        Boolean(cuidados.dispostoVeterinario)
      ),
    ...
    try {
      ...
      setAlertaParams({
        mensagem: "Falha ao registrar a adoção",
        severity: "error",
      });
      setAlertaOpen(true);
    }
    ...
    switch (step) {
      case 0:
        return (
          <FormDadosPet
            ...
            onChangePetId={setSelectedPetId}
          />
        );
      case 1:
        return <FormAdotante />;
      case 2:
        return (
          <FormFamilia
            values={familia}
            onChange={(p) => setFamilia((prev) => ({ ...prev, ...p }))}
          />
        );
      case 3:
        return (
          <FormMoradia
            values={moradia}
            onChange={(p) => setMoradia((prev) => ({ ...prev, ...p }))}
          />
        );
      case 4:
        return (
          <FormCuidados
            values={cuidados}
            onChange={(p) => setCuidados((prev) => ({ ...prev, ...p }))}
          />
        );
      default:
        return null;
